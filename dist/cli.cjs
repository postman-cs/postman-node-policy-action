...
function normalizeRelPath(rootDir, fullPath) {
  return (0, import_node_path.relative)(rootDir, fullPath).split(import_node_path.sep).join("/");
}
function toAbsolute(rootDir, relPath) {
  return (0, import_node_path.join)(rootDir, ...relPath.split("/"));
}
...
function normalizeMinimumVersion(input) {
  const parsed = import_semver.default.coerce(input);
  if (!parsed) {
...
function minimumEngineRange(input) {
  const trimmed = input.trim();
  if (/^\d+$/.test(trimmed)) {
    return `>=${trimmed}`;
  }
  const parsed = import_semver.default.coerce(trimmed);
  if (!parsed) {
    throw new Error(`minimum-node-version must be a Node version, got: ${input}`);
  }
  const normalized = parsed.patch === 0 && parsed.minor === 0 ? String(parsed.major) : parsed.version;
  return `>=${normalized}`;
}
function lineFor(contents, needle) {
  const index = contents.indexOf(needle);
  if (index < 0) return 1;
  return contents.slice(0, index).split(/\r?\n/u).length;
}
function isFloatingNodeValue(value) {
  return /^(node|latest|current|\*|lts\/\*)$/iu.test(value.trim());
}
function nodeVersionMeetsMinimum(value, minimumMajor) {
  const trimmed = value.trim().replace(/^v/iu, "");
  const parsed = import_semver.default.coerce(trimmed);
  if (!parsed) return void 0;
  return parsed.major >= minimumMajor;
}
function nodeRuntimeMeetsMinimum(value, minimumMajor) {
  const match = /^node(\d+)$/iu.exec(value.trim());
  if (!match) return void 0;
  return Number(match[1]) >= minimumMajor;
}
function rangeHasCompatibleVersion(range, minimumRange) {
  try {
    return import_semver.default.intersects(range, minimumRange, { includePrerelease: true });
  } catch {
    return false;
  }
}
function rangeStaysAtOrAboveMinimum(range, minimumRange) {
  try {
    const minimum = import_semver.default.minVersion(minimumRange);
    const rangeMinimum = import_semver.default.minVersion(range);
    return Boolean(minimum && rangeMinimum && import_semver.default.gte(rangeMinimum, minimum));
  } catch {
    return false;
  }
}
function hasLowerNodeRange(range, context) {
  return !rangeStaysAtOrAboveMinimum(range, context.minimumRange);
}
function dependencyRangeViolates(range, context) {
...
function isIgnoredByFileScan(relPath, ignored) {
  ...
  return hasDefaultIgnoredSegment(relPath) || ignored.has(relPath) || ignored.has(firstSegment) || [...ignored].some((ignore) => relPath.startsWith(`${ignore}/`));
}
function relDir(file) {
  return (0, import_node_path.dirname)(file) === "." ? "." : (0, import_node_path.dirname)(file);
}
function npmPrefixForPackageFile(file) {
  const packageDir = relDir(file);
  return packageDir === "." ? "npm" : `npm --prefix ${packageDir}`;
}
function packageEngineFixCommand(file, context) {
...
async function readText(rootDir, file) {
  return (0, import_promises.readFile)(toAbsolute(rootDir, file), "utf8");
}
function scanPackageJson(file, contents, context) {
  const violations = [];
  let parsed;
  try {
...
function matrixValuesFor(job, expression) {
  ...
  if (Array.isArray(value)) {
    return value.map(asString).filter((entry) => Boolean(entry));
  }
  const single = asString(value);
  return single ? [single] : [];
}
async function scanWorkflow(file, contents, context) {
  const violations = [];
  let parsed;
  try {
    parsed = (0, import_yaml.parse)(contents);
        ...
        for (const valueToCheck of valuesToCheck) {
          ...
          if (meetsMinimum === false || meetsMinimum === void 0) {
            violations.push(makeViolation({
              ...
              expected: context.preferredMajor,
              fixable: matrixValues.length === 0,
              fix: matrixValues.length === 0 ? `Set node-version to ${context.preferredMajor}.` : `Remove Node ${valueToCheck} from the setup-node matrix or replace it with ${context.preferredMajor}.`
            }));
          }
        }
      }
...
function scanDockerfile(file, contents, context) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  for (const [index, line] of lines.entries()) {
...
function scanPackageLock(file, contents, context) {
...
function scanPnpmLock(file, contents, context) {
  if (!context.options.scanDependencies) return [];
  const violations = [];
  let parsed;
  try {
...
async function packageManifestExists(packageDir) {
  try {
    return (await (0, import_promises.stat)((0, import_node_path.join)(packageDir, "package.json"))).isFile();
  } catch {
    return false;
  }
}
async function collectInstalledPackageManifests(rootDir, packageDir = ".") {
  ...
  async function collectNodeModules(nodeModulesDir) {
    let entries;
    try {
      entries = await (0, import_promises.readdir)(nodeModulesDir, { withFileTypes: true });
    } catch {
      return;
    }
    for (const entry of entries) {
      if (!entry.isDirectory() || entry.name === ".bin") continue;
      const fullPath = (0, import_node_path.join)(nodeModulesDir, entry.name);
      if (entry.name.startsWith("@")) {
        const scopedEntries = await (0, import_promises.readdir)(fullPath, { withFileTypes: true });
        for (const scopedEntry of scopedEntries) {
          if (scopedEntry.isDirectory()) {
            await collectPackage((0, import_node_path.join)(fullPath, scopedEntry.name));
          }
        }
        continue;
      }
...
async function applyFixes(violations, context) {
  ...
  for (const violation of violations) {
    ...
    const contents = await (0, import_promises.readFile)(fullPath, "utf8");
    if (violation.kind === "package-engines") {
      const parsed = JSON.parse(contents);
      const engines = asRecord(parsed.engines) ?? {};
      engines.node = context.minimumEngineRange;
      parsed.engines = engines;
      await (0, import_promises.writeFile)(fullPath, `${JSON.stringify(parsed, null, 2)}
`);
      changed.add(violation.file);
      continue;
    }
    if (violation.kind === "node-version-file") {
      await (0, import_promises.writeFile)(fullPath, `${context.preferredMajor}
`);
      changed.add(violation.file);
      continue;
    }
...
function formatSummary(result, context, initialViolationCount) {
  const lines = [];
...
function suggestedCommandsForViolations(violations, context) {
...
async function checkNodePolicy(options) {
  const rootDir = options.rootDir;
  const rootStat = await (0, import_promises.stat)(rootDir);
